...
均值、众数、中位数、方差、标准差，这些是你可以用来描述数据的一些统计数据，就像在概率中一样，除非你可以访问所有可用的数据(总体)，否则一个**“统计数据”**总是从一个样本中获得:如果你可以从总体中获得它，你会称它为一个**“参数”**。根据这些信息，可以建立预测模型，并在结果真正实现之前确定某些结果的**概率**。这样，你就可以用统计学来计算出某一事件发生的概率。
