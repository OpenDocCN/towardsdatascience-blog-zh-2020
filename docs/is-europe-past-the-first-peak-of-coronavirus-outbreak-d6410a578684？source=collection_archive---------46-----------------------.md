...
![](img/3339c8e214eb2e6bb0074f9131318c0a.png)

安娜斯塔西娅·切平斯卡在 [Unsplash](https://unsplash.com/s/photos/covid-europe?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) 上的照片
...
[](https://www.washingtonpost.com/world/europe/europe-is-emerging-from-the-pandemic-its-hospitals-stable-with-beds-available/2020/04/17/c7aa46cc-7f37-11ea-84c2-0792d8591911_story.html) [## 欧洲认为疫情冠状病毒的高峰期已经过去
...
在本文中，我将介绍几种数据可视化技术，使用 Python Plotly 库来可视化新冠肺炎数据，并根据从 Kaggle 获得的数据集观察五个选定的欧洲国家(英国、法国、德国、意大利和西班牙)的疫情进展状态。为了实现这个目标，我将一步一步地介绍如何使用 Python Plotly 创建**时间序列折线图、条形图和量表图来检查欧洲的新冠肺炎记录。**
...
[冠状病毒报告](https://www.kaggle.com/imdevskp/corona-virus-report)(来自 Kaggle)
...
![](img/484066ed10a77eae389c3e9b4d7f6fe3.png)

前 20 条记录(左)和后 20 条记录(右)

首先，我们使用熊猫*头*和*尾*方法(第 5 行& 6)来预览前 20 行和后 20 行记录。从预览图中，我们知道该数据涵盖了从 2020 年 1 月 22 日到 2020 年 4 月 20 日全球每日确诊、死亡和恢复的新冠肺炎病例的**累积总和。**
...
![](img/51e736474e84602e3dd9daa030eb897c.png)
...
![](img/0e5c72c884066436c08988afbaeacd6b.png)

汇总数据

我们可以使用 Pandas data frame***group by***方法对我们的日期和国家进行分组，并使用 ***agg*** 方法根据每个国家的每日确诊、死亡和康复病例总数汇总数据。

## 1.4 转换数据

在这个阶段，我们已经成功地过滤了数据，并根据日期对它们进行了排序。在我们继续绘制图表之前，还有一个步骤。每行记录中的确诊、死亡和康复病例数为**之前的累计数**。我们的目的是绘制一些时间序列图表，这些图表可以揭示每日新报告病例的进展情况，而不是累积的总和。从逻辑上讲，我们应该用前一个日期的数据减去当前日期的数据，以得出每日新病例的数量。Pandas 使我们能够使用 ***diff*** 方法转换数据(第 24、27、30 行)。

转换数据的 Python 代码

![](img/e392b724668b244e7903227b16c0a740.png)

转换数据

我们将转换后的数据分配给三个新列，分别为 *Confirmed_Plus* 、 *Death_Plus、*和 *Recovered_Plus、**。*这三栏用于保存每日新增确诊病例、死亡病例和康复病例的记录(为**而非**累计数)。
...
现在，我们准备使用[**Python Plotly Express**](https://plotly.com/python/plotly-express/)**包绘制我们的第一个折线图(第 27–28 行)。**
...
**![](img/3b5b1e9cff4e59697d87e71fb490d4bb.png)**
...
**Plotly 库提供的另一个有用的图表是仪表图。[仪表图](https://plotly.com/python/gauge-charts/)是一种显示单个值的指示器，用于估计目标进度。在这种情况下，我们假设每个国家的目标都是完全恢复。考虑到这一点，我们可以尝试为每个国家创建一个量表，以显示他们实现目标的程度。**

**要创建仪表图，我们只需**

*   **将 2020 年 4 月 20 日的累计回收金额设置为参数*值*(第 7 行)。**
*   **为图表设置一个相关的标题(第 9 行)。**
...
**![](img/aa753a8f9f5ff89382e68e30ff94e8ad.png)**

**意大利仪表图**

**为德国创建仪表图的 Python 代码**

**![](img/e4c18353f32f3cf0e1e769dfa9e5ea9a.png)**

**德国仪表图**

**为西班牙创建仪表图的 Python 代码**

**![](img/8240a896cad0fae2aa6f87c902f39bd4.png)**

**西班牙仪表图**

**为法国创建仪表图的 Python 代码**

**![](img/79837aa12acbddf26e98fdc1ecdf0bb4.png)**
...
**这里展示的所有代码都可以在我的 [GitHub 库](https://github.com/teobeeguan/COVID-19-TimeSeriesCharts)中找到。**

# **参考**

1.  **[https://www . Washington post . com/world/Europe/Europe-is-emerging-from-the-the-疫情-its-hospitals-stable-with-beds-available/2020/04/17/c7aa 46 cc-7f 37-11ea-84 C2-0792d 8591911 _ story . html](https://www.washingtonpost.com/world/europe/europe-is-emerging-from-the-pandemic-its-hospitals-stable-with-beds-available/2020/04/17/c7aa46cc-7f37-11ea-84c2-0792d8591911_story.html)**
2.  **[https://plotly.com/python/plotly-express/](https://plotly.com/python/plotly-express/)**
3.  **[https://plotly.com/python/gauge-charts/](https://plotly.com/python/gauge-charts/)**